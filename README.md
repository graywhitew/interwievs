...
> __SOLID__:  
>  S : __Single Responsibility Principle__ (Принцип единственной ответственности) каждый класс должен выполнять только те цели, для которых он был задуман.  
>  O: __Open-Closed Principle__ (Принцип открытости-закрытости) класс должен быть открыт для рассширения и закрыт для модификации(только через наследование).  
>  L: __Liskov Substitution Principle__ (Принцип подстановки Барбары Лисков) если у нас есть родительский и дочерний класс, и у нас есть функция умеющая работать с родительским классом, мы в эту функцию можем передать объект дочернего класса и это будет работать, потому что объект дочернего класс должен полностью уметь заменять родительский класс.  
>  I: __Interface Segregation Principle__ (Принцип разделения интерфейса) чем иметь один огромный интерфейс под всё что только можно, лучше иметь много мелких интерфейсов под конкретные задачи.  
>  D: __Dependency Inversion Principle__ (Принцип инверсии зависимостей) должны быть зависимости на классах, а не классы на зависимостях.  
> __KISS__ - "Keep is simple, stupid" надо держать свои функции максимально простыми, чтобы они выполняли только то что нужно им, не более.  
> __DRY__ - "Don't repeat your self" чем больше раз мы повторяем свой кодиарник, то если возникнет необъ. его модифицировать, то в n местах где мы его повторили n раз его надо будет промодифицировать, т.е. n вероятность ошибки, поэтому по хорошему повторяющийся код надо писать в одном месте выносить в функцию.  
> __YAGNI__ - "You are gonna need it" в коде должен быть только тот функционал, который нужен тебе сейчас.  
...
> __MongoDB__, __Redis__, __RavenDB__, __Couchbase__ и т.д.  

- Какие SQL базы данных знаешь, с какими работал?
> __MySQL__ и __PostgreSQL__.
> Какие отличия __MySQL__ от __PostgreSQL__?  
> Отличие __MySQL__ от __PostgreSQL__ заключается в том, что первая система рассчитана на проекты с интенсивным чтением данных, для которых важна скорость и легкость управления, а
> вторая подходит для сложных запросов и работы с большими объемами информации (Big Data). У них разные подходы к хранению данных и их обработке, они отличаются производительностью
> и количеством поддерживаемых типов данных (в Postgre их больше).

- PK, FK
> __PrimaryKey(первичный ключ)__ - уникальный ключ, который однозначно идентифицирует строку в таблице.  
> __ForeignKey__ - ключ, использующийся для связи таблиц между друг другом.  
...
> __Триггеры__ - event handlers, которыми служат какие то события и потом по этим событиям можно выполнять какие то действия, например слушаем обновление таблицы, и когда она обновилась мы поле update_date меняем на текущую дату.

- Что такое транзакция?
> __Транзакция__ - блок операций (запрос к БД), в котором будут выполнены все операции, либо ниодной, но есть такие операции, которые неявно делают коммит транзакции(DROP TABLE, TRUNCATE TABLE, ALTER TABLE)

- ACID
> __Atomicity__ — Атомарность | Транзакция не может быть зафиксированна частично, будет выполнена полностью  
> __Consistency__ — Согласованность | Допустимость результатов данных  
> __Isolation__ — Изолированность | Одна транзакция не может вклинуться в другую транзакцию  
> __Durability__ — Надёжность | Если транзакция завершена, то результат зафиксирован в системе и ничего на это уже повлиять не может (никак внешний фактор)  
...
>__Read committed__. Для этого уровня параллельно исполняющиеся транзакции видят только зафиксированные изменения из других транзакций. Таким образом, данный уровень обеспечивает защиту
...
# Tools and Cloud
- Git
> Git является распределенной системой контроля версий. Распределенная система контроля версий - это система, при которой копия репозитория может храниться на машине у каждого
> разработчика, что значительно снижает риск потерять результат работы над проектом.

```git
git init: Создает новый репозиторий Git в текущем каталоге или инициализирует существующий каталог как репозиторий.

git commit: Фиксирует изменения в индексированных файлах и создает новый коммит в истории репозитория.

git clone: Клонирует удаленный репозиторий на локальную машину, создавая копию репозитория.
...
git reset: Позволяет отменить коммиты, перемещая указатель текущей ветки на другой коммит.(soft, mixed, hard)
...
git log: Отображает список коммитов в репозитории с их хешами, авторами, датами и сообщениями.

git show: Показывает информацию о конкретном коммите, включая изменения файлов.

git reflog: Отображает историю всех операций с указателями веток и коммитами.

git merge: Объединяет две или более ветки, применяя изменения из одной ветки в другую.

git branch: Отображает список веток в репозитории или создает новую ветку.

git add: Добавляет изменения из рабочей директории в индекс (подготовка к коммиту).

git status: Показывает состояние рабочей директории и индекса, указывая на добавленные, измененные или неотслеживаемые файлы.

git checkout: Переключает между ветками или коммитами, восстанавливает файлы из определенных состояний.

git rebase: Перемещает или перебазирует текущую ветку на другую ветку или коммит. Он позволяет применить изменения из одной ветки поверх другой, создавая впечатление последовательного коммитирования. Это может помочь вам поддерживать более чистую историю коммитов.

git cherry-pick: Применяет изменения (коммиты) из другой ветки на текущую ветку. Это может быть полезно, если вы хотите выбрать и применить конкретные коммиты из другой ветки, не объединяя всю ветку.
```
> Чем отличаются __rebase__, __cherry-pick__ и __merge__?  
> Если история изменений отличается, то  merge создаст новый __commit__, __rebase__ примет их подряд по времени исполнения, а __cherry-pick__ это просто взять __commit__ и применить его к какой то ветке в конце.

> Файлы бывают:  
> __Untracked__ (Неотслеживаемые): Это состояние, когда Git еще не начал отслеживать файлы. Это может быть новый файл, который вы создали, но еще не добавили в индекс.  
> __Modified__ (Измененные): Это состояние, когда файл уже отслеживается Git, но были внесены изменения. Файлы находятся в состоянии "измененных", когда вы внесли изменения после
> последнего коммита.  
> __Staged__ (Индексированные): После того как вы внесли изменения в файлы, вы можете добавить их в "индекс", также известный как "стейдж". Файлы в этом состоянии готовы к коммиту.  
> __Committed__ (Зафиксированные): Это состояние, когда изменения в файлах были зафиксированы в истории коммитов Git. Коммит создает точку в истории, сохраняя состояние файлов в этот момент.

- Docker
> Docker - это инструмент, который упрощает управление приложениями и их зависимостями, обеспечивая портабельность и изоляцию.

> __Docker__ vs __Виртуальная машина__ (ВМ):
> __Docker__ использует концепцию контейнеризации, где каждое приложение и его зависимости запускаются в изолированных контейнерах, разделяющих общее ядро операционной системы. Это
> делает контейнеры более легковесными и быстрыми в запуске, по сравнению с __ВМ__.  
> __Виртуальные машины__ полностью эмулируют отдельные виртуальные компьютеры, каждый с собственной операционной системой. Они более изолированны и могут работать с разными ОС, но
> требуют больше ресурсов и времени для запуска.

> __Docker Image__: __Docker Image__ - это шаблон или образ контейнера, который содержит исполняемый код, зависимости и другие компоненты, необходимые для запуска приложения.
> __Docker Container__: __Docker Container__ - это экземпляр Docker Image, который работает в изолированном окружении. Контейнеры обеспечивают легковесную и консистентную среду для
> выполнения приложений.  
> __Docker Daemon__: __Docker Daemon__ - это фоновый процесс, который управляет операциями Docker на хостовой машине. Он обрабатывает запросы API Docker и управляет контейнерами,
> образами и другими ресурсами.  
> __Docker Registry__: __Docker Registry__ - это хранилище Docker Images. Один из наиболее известных регистров - Docker Hub. Вы также можете создать собственный частный реестр для
> хранения образов.  
> __Dockerfile__: __Dockerfile__ - это текстовый файл, который содержит инструкции для создания Docker Image. Он определяет базовый образ, устанавливает зависимости, копирует файлы и
> выполняет другие задачи для подготовки образа.
> __.dockerignore__: Файл __.dockerignore__ позволяет указать файлы и директории, которые Docker не должен копировать в контейнер при создании образа.


> Основные команды __Docker__: 
> __docker build__: Создание __Docker Image__ из __Dockerfile__.  
> __docker run__: Запуск контейнера из __Docker Image__.  
> __docker ps__: Просмотр запущенных контейнеров.  
> __docker images__: Просмотр списка доступных __Docker Images__.  
> __docker pull__: Загрузка __Docker Image__ из __Docker Registry__.  
> __docker push__: Загрузка __Docker Image__ в __Docker Registry__.  
> __docker exec__: Выполнение команды внутри запущенного контейнера.  
> __docker stop__, __docker start__: Остановка и запуск контейнера.  
