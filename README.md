...
```
> Чем отличаются rebase, cherry-pick и merge?
> Если история изменений отличается, то  merge создаст новый commit, rebase примет их подряд по времени исполнения, а cherry-pick это просто взять commit и пременинть его к какой то
> ветке в конце.
